...
import { getAnalysesStatus } from '../../../../app/components/backend/analyses'
...
export default async function handler (req, res) {
  ...
  if (req.method === 'GET') {
    const resp = await getAnalysesStatus(projectId, session.user.token)
    res.status(resp.status).json(resp.body)
  } else {
    res.status(200).json({ name: 'api/analyses/status' })
  }
}
