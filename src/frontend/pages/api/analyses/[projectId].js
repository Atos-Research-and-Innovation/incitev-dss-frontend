...
import { createAnalysis } from '../../../app/components/backend/analyses'
...
export default async function handler (req, res) {
  ...
  if (req.method === 'POST') {
    const resp = await createAnalysis(projectId, req.body, session.user.token)
    res.status(resp.status).json(resp.body)
  } else {
    res.status(200).json({ name: 'api/analyses' })
  }
}
