...
import { getDCMCharging } from '../../../app/components/backend/dcm'
...
export default async function handler (req, res) {
  ...
  if (req.method === 'POST') {
    const resp = await getDCMCharging(req.body, session.user.token)
    res.status(resp.status).json(resp.body)
  } else {
    res.status(200).json({ name: 'api/dcm/charging' })
  }
}
