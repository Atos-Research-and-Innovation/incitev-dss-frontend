...
import { getUserMe, putUserMe } from '../../../app/components/backend/users'
...
export default async function handler (req, res) {
  ...
  if (req.method === 'GET') {
    const resp = await getUserMe(session.user.token)
    res.status(resp.status).json(resp.body)
  } else if (req.method === 'PUT') {
    const resp = await putUserMe(req.body, session.user.token)
    res.status(resp.status).json(resp.body)
  } else {
    res.status(200).json({ name: 'api/users/me' })
  }
}
