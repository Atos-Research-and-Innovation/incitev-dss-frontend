...
import { getResults } from '../../../../app/components/backend/results'

export default async function handler (req, res) {
  const { projectId, analysisId } = req.query
  if (req.method === 'GET') {
    const resp = await getResults(projectId, analysisId, 0)
    res.status(resp.status).json(resp.body)
  } else {
    res.status(200).json({ name: `api/results/${projectId}/${analysisId}` })
  }
}
