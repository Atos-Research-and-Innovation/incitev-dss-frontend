...
export async function getDCMCharging (values, token) {
  const res = await fetch(
    process.env.DSS_BACKEND_BASE_URL + 'dcm/charging',
    {
      method: 'POST',
      mode: 'no-cors',
      cache: 'no-store',
...
export async function getDCMRatio (values, token) {
  const res = await fetch(
    process.env.DSS_BACKEND_BASE_URL + 'dcm/ratio',
    {
      method: 'POST',
      mode: 'no-cors',
      cache: 'no-store',
