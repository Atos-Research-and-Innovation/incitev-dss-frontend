...
import AnalysisList from './components/AnalysisList.js'
import Refresher from './components/Refresher.js'

export default function AnalysisHome ({ params }) {
  const { city, projectId } = params

  return (
    <CommonNav title={city} refresh addIcon addUrl={`/${city}/${projectId}/analysis/add`}>
      <Refresher />
      <AnalysisList city={city} projectId={projectId} />
    </CommonNav>
  )
}
